...
# Use Raspberry Pi OpenCV haarcascade path (Debian package)
...
# Create directory for captured images (if you ever want to save locally)
...
def send_image_to_client(frame, timestamp, num_faces):
    ...
    try:
        ...
        # Header format:
        #   b"IMG" + 14-byte timestamp (utf-8) + 4-byte big-endian size
...
def detect_and_capture_faces():
    ...
    global face_detection_enabled, last_face_detected_time, camera
...
while True:
    print('New data from %s:%d' % (addr[0], addr[1]))
    ...
    # Echo back command (for the client to see confirmation)
