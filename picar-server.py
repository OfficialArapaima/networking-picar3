...
# Functions from Video Recording Code 
def print_overwrite(msg, end='', flush=True):
    # This function is useful for status updates on the Pi's console
    print('\r\033[2K', end='', flush=True)
    print(msg, end=end, flush=True)
# Vilib Camera Setup
def initialize_camera_and_recording(): 
    username = os.getlogin()
    Vilib.rec_video_set["path"] = f"/home/{username}/Videos/" 

    # Start the camera and display for both tracking and recording
    Vilib.camera_start(vflip=False, hflip=False)
    Vilib.display(local=True, web=True)
    Vilib.face_detect_switch(False) # Start tracking off
    px.set_cam_pan_angle(0)
    px.set_cam_tilt_angle(0)
    sleep(0.8)
    print("Vilib Camera and Display initialized.")
# Video Recording
def handle_recording_command(command):
    ...
    if command == 'q':
        if rec_flag == 'stop':
            ...
            # Start record: must call run() then start()
            Vilib.rec_video_run()
    ...
    elif command == 'e' and rec_flag != 'stop':
        rec_flag = 'stop'
        Vilib.rec_video_stop()
        response = "The video saved as %s%s.avi" % (Vilib.rec_video_set["path"], vname)
